...
void tick_handler(struct tm *tick_time, TimeUnits units_changed){
  if (units_changed & DAY_UNIT) {
    ...
    snprintf(date_string, sizeof(date_string), "%s %d", 
      dname[current_tm->tm_wday], current_tm->tm_mday);
...
static void window_load(Window *window) {
  ...
  time_outline_layer = text_layer_create(GRect(0, 142, 144, 30));
  ...
  time_text_layer = text_layer_create(GRect(0, 142, 144, 30));
  ...
  //date outline
  date_outline_layer = text_layer_create(GRect(0, -4, 144, 18));
